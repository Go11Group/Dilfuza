...
type CourseRepo struct { //bu ma'lumotlar bazsidagi course tebil bilan o'zaro ishlash 
	DB *sql.DB
}
...
func (db *CourseRepo) CreateCourse(req *models.Course) error { //CreateCourse kursni ma'lumotlar bazasiga qo'shadigan funksiya.

	_, err := db.DB.Exec(
		`insert into courses(name, email) values($1, $2)`, req.Title, req.Description)
	return err
}
func (db *CourseRepo) ById(id string) (*models.Course, error) {   // ById kursni ID boyicha qidiruv va olish uchun funksiya.
...
func (db *CourseRepo) GetCoursesByUser(userID string) (*[]models.Course, error) {  // GetCoursesByUser foydalanuvchi ID si bo'yicha kurslarni olish uchun funksiya.
